...
export let getUsrMsg = (id) => {
    return request('get', '/api/base/user/find', { id });
};

// 查询组织部门tree
export let updataUsrMsg = (parmas) => {
    return request('post', '/api/base/user/info/update', parmas);
};
...
// 上传头像地址
export let imgUrl = window.ShiYue.base + '/api/storage/upload/type';
...
//修改某个模块
export let editgPortallet = (id, parmas) => {
