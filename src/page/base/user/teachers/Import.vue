        ...
        <div slot="bodyRight">
            ...
            <div class="sy-import sy-down">
                ...
                <div class="sy-import-parms">
                    <el-checkbox v-model="sendSms" v-if="isShowMessBtn">导入成功是否发短信给教师</el-checkbox>
                    <div class="info-msg">
                        <p style="padding:0 8px;margin:0;">编辑短信内容：{账号}、{密码}会自动把用户的账号和密码填充进去</p>
...
import {downloadImportColumns,
        downloadImportTemplate,
        downloadParams,
        checkMessage,
        downloadTeacherImportPass,
        importTeachers} from '../request.js';
export default {
    components: {
    ...
    created(){
        ...
        checkMessage().then((data)=>{
            console.log('>>>>>',data);
            this.isShowMessBtn = data ? true : false;
        })
    ...
    methods: {
        back(){
            this.$router.push({ name: 'teachers', params: { unitId: this.currentUnitId, currentNodeKey: this.currentNodeKey,unitType: this.unitType } });
        },
        submitUpload() {
            if(!this.uploadValue){
                this.$notify({
