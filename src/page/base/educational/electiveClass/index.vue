        ...
        <sy-grad-4 :border="false">
          ...
          <div slot="bodyRight">
            <el-tree
              ...
              highlight-current
              :default-expanded-keys="sess.arr"
              :current-node-key="sess.unitId"
        ...
        <sy-grad-4 :border="false" headerH="40px">
          <div slot="headerRight" class="left1">
            <span class="sy-row" gutter="5px">
              ...
              <span class="sy-col sy-4">
                <el-select v-model="valueYear" size="small" placeholder="学年" :disabled="!btn_canClick"
                           @change="onChangeSelect('year')" style="margin-right: 10px">
                  <el-option
                    v-for="year in years"
                    :key="year.id"
                    :label="year.year"
              ...
              <span class="sy-col sy-4">
                <el-select v-model="valueTerm" size="small" placeholder="学期" :disabled="!btn_canClick"
                           @change="onChangeSelect('term')" style="margin-right: 10px">
                  <el-option
                    v-for="term in terms"
                    :key="term.schoolTerm"
                    :label="term.name"
              ...
              <span class="sy-col sy-4">
                <el-select v-model="valueClass" size="small" placeholder="课程类型" :disabled="!btn_canClick"
                           @change="onChangeSelect('class')" style="margin-right: 10px">
                  <el-option
              ...
              <span class="sy-col sy-4">
                <el-select v-model="valueGrade" size="small" placeholder="年级" :disabled="!btn_canClick"
                           @change="onChangeSelect('grade')" style="margin-right: 10px">
                  <el-option
              ...
              <span class="sy-col1 sy-7">
                <!--<span class="sy-col1 sy-6">-->
                  <el-button v-if="codeList.btn_base_electivecourse_importstudent"
                             :disabled="(!btn_canClick)||isCurrentTerm" size="small" @click="importSs"
                             style="float:right; margin-left:10px">导入学生</el-button>
                <!--</span>-->
                <!--<span class="sy-col1 sy-4">-->
                  <el-button v-if="codeList.btn_base_electivecourse_importclass"
                             :disabled="(!btn_canClick)||isCurrentTerm" size="small" @click="importCs"
                             style="float:right; margin-left:10px">导入</el-button>
                <!--</span>-->
                <!--<span class="sy-col1 sy-4">-->
                  <el-button v-if="codeList.btn_base_electivecourse_addclass" :disabled="(!btn_canClick)||isCurrentTerm"
                             size="small" @click="newElClass"
                             style="float:right; margin-left:10px">新建</el-button>
                <!--</span>-->
                <!--<span class="sy-col1 sy-1-1">-->
                <!--<el-button size="small" @click="exportSs" style="width:100%">导出学生</el-button>-->
                <!--</span>-->
                <!--<span class="sy-col1 sy-4">-->
                  <el-button v-if="codeList.btn_base_electivecourse_copy" :disabled="(!btn_canClick)||isCurrentTerm"
                             size="small"
                             @click="isvisible_copy=true, valueTerm2='', valueYear2='', tableData_copy=[]"
                             style="float:right; margin-left:10px">复制</el-button>
                <!--</span>-->
              </span>
            </span>
          </div>
          <div slot="bodyRight" style="width:100%; height:100%; padding:0 10px 0 10px">
            <el-table
              ...
              <el-table-column
                v-if="codeList.btn_base_electivecourse_delete"
                type="selection"
                width="48px">
              </el-table-column>
              <el-table-column
              ...
              <el-table-column label="操作" align="center" width="120px">
                <template scope="scope">
                  ...
                  <el-button
                    v-if="codeList.btn_base_electivecourse_edit"
                    size="small"
                    type="text"
                  ...
                  <!--</el-tooltip>-->
                  <!--<el-tooltip effect="light" content="分配学生" placement="right">-->
                  <el-button
                    v-if="codeList.btn_base_electivecourse_fenpei"
                    size="small"
                    type="text"
                    @click="handleDistribution(scope.$index, scope.row)">
                    <i class="fa fa-hand-o-right" title="分配学生" aria-hidden="true" style="font-size: medium"></i>
                  </el-button>
                  <!--</el-tooltip>-->
                  <!--<el-tooltip effect="light" content="查看选修学生" placement="right">-->
                  <el-button
                    v-if="codeList.btn_base_electivecourse_chakan"
                    size="small"
                    type="text"
                    @click="handleDistributionStudent(scope.$index, scope.row)">
                    <i class="el-icon-search" title="查看选修班学生"></i>
                  </el-button>
                  <!--</el-tooltip>-->
                </template>
              </el-table-column>
          ...
          <div slot="footer" style="padding-top:3px">
            <span>
              <el-button v-if="codeList.btn_base_electivecourse_delete" size="small" type="danger" style="float: left"
                         :disabled="(!btn_canClick)||isCurrentTerm || multipleSelection.length < 1"
...
//          this.sess = JSON.parse(window.sessionStorage.getItem('importElClass'));
...
//              this.sess.unitId = data.data[0].unitId;
                ...
                for (let i = 0; i < tree.length; i++) {
                  if (tree[i].children && tree[i].children.length !== 0) {
                    ...
                    if (tree[i].unitType === 2) {
                      tree[i].open = 1;
//                      this.sess.unitId = tree[i].unitId;
...
//                for (let _c of cData) {
//                  if (_c.nodeAttribute === "course_type") {
//                    if (!Boolean(_c.children) || _c.children.length < 1) {
//                      _c.children = [];
//                      _c.children.push({name: "此课程类型下无课程!",id:"", disabled: true});
//                    }else{
//                      c(_c.children);
//                    }
//                  }
...
//        if (val.nodeAttribute === "course") {
//          this.isvisible_choose = false;
//          this.elClassData.courseId = val.id;
...
//        }
      ...
      handleEdit(index, row){
        ...
        this.editShortName = row.shortName;
        this.dialogTitle = "修改";
        ...
        config.getPlaceType(this.unitId).then((data) => {
          if (data && data.data && data.data.length > 0) {
            ...
            for (let i = 0; i < data.data.length; i++) {
              this.places[i].children = []
            }
          }
        });
        config.beforeEdit(this.tableData[index].id).then((data) => {
          if (data.data) {
            this.elClassData = data.data;
            this.elClassData.courseTerm = this.elClassData.courseTerm + "";
            this.elClassData.courseTeacher = this.elClassData.courseTeacherIds;
            this.courseName = !(this.elClassData.courseName === null || this.elClassData.courseName.length < 1) ? this.elClassData.courseName : "点击选择";
            this.gradeNames = !(this.elClassData.gradeNames === null || this.elClassData.gradeNames.length < 1) ? this.elClassData.gradeNames : "可选择多个";
            this.teacherNames = !(this.elClassData.courseTeacherNames === null || this.elClassData.courseTeacherNames.length < 1) ? this.elClassData.courseTeacherNames : "可选择多个";
            this.className = !(this.elClassData.classRoomName === null || this.elClassData.classRoomName.length < 1) ? this.elClassData.classRoomName : "点击选择";
            this.cIds.push(this.elClassData.courseId);
            if (this.elClassData.gradeIds && this.elClassData.gradeIds.length > 0) {
              this.gradeIds = this.elClassData.gradeIds.split(",");
            }
            this.$refs.grade.setCheckedKeys(this.gradeIds);
            this.$refs.course.setCheckedKeys(this.cIds);
          }
        })
      },
      ...
      // 查看选修班学生
      handleDistributionStudent(index, row){
        this.isvisible_see = true;
        this.exportId = row.id;
        this.tableData2 = [];
        config.getStudents({
          unitId: this.unitId,
          ...
          if (data && data.data && data.data.length > 0) {
            for (let i = 0; i < data.data.length; i++) {
              data.data[i].gender += "";
              data.data[i].gender = data.data[i].gender == '1' ? '男' : (data.data[i].gender == '2' ? '女' : "");
            }
...
//        if(this.addOrEdit)
//          this.elClassData.courseId = this.courseId[this.courseId.length - 1];
//        else
//          this.elClassData.courseId = this.courseId[this.courseId.length - 1]&&this.courseId[this.courseId.length - 1].length>0?this.courseId[this.courseId.length - 1]:this.elClassData.courseId;
        this.$refs.addData.validate((valid) => {
          if (valid) {
            let doc = document.getElementsByClassName('sy-dialog');
            let loadingInstance = Loading.service({
              target: doc[doc.length - 5]
            })
            if (this.addOrEdit) {
...
//        this.$router.push({path: '/base/elective_importSs/' + this.listParams.unitId});
//        this.$setSess('importElClass', this.sess);
      },
...
//        this.$router.push({path: '/base/elective_importCs/' + this.listParams.unitId});
//        this.$setSess('importElClass', this.sess);
      }
    }
  }
